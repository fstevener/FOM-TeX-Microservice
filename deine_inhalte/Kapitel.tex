...
Abbildung ~\ref{fig:schemaversionierung} zeigt das aus den DDL-Statements in Quelltext ~\ref{lst:ddl} resultierende Schema bei einem DBMS, das Schema-Versionierung unterstützt. Im Gegensatz zur Schema-Evolution koexistiert die neue Schemaversion $\mathrm{SV}_2$ mit der ursprünglichen Schemaversion $\mathrm{SV}_1$ innerhalb des DBMS. Dies wird unter anderem durch die beiden Einträge im Systemkatalog deutlich. Die Datenbankinstanz $I_1$ der Schemaversion $\mathrm{SV}_1$ bleibt vollständig erhalten. Für $\mathrm{SV}_2$ werden die Daten analog zur Schema-Evolution an die neue Schemastruktur angepasst, sodass $I_2$ entsteht. Dadurch können Anwendungen, die entweder gegen $\mathrm{SV}_1$ oder gegen $\mathrm{SV}_2$ entwickelt wurden, weiterhin ohne Anpassungen betrieben werden.
